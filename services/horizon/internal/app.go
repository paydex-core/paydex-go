...
import (
	...
	"github.com/paydex-core/paydex-go/services/horizon/internal/expingest"
	"github.com/paydex-core/paydex-go/services/horizon/internal/ingest"
	"github.com/paydex-core/paydex-go/services/horizon/internal/ledger"
	"github.com/paydex-core/paydex-go/services/horizon/internal/logmetrics"
	"github.com/paydex-core/paydex-go/services/horizon/internal/operationfeestats"
	"github.com/paydex-core/paydex-go/services/horizon/internal/paths"
	"github.com/paydex-core/paydex-go/services/horizon/internal/reap"
	"github.com/paydex-core/paydex-go/services/horizon/internal/txsub"
	"github.com/paydex-core/paydex-go/support/app"
...
// UpdatePaydexCoreInfo updates the value of coreVersion,
// currentProtocolVersion, and coreSupportedProtocolVersion from the Paydex
// core API.
func (a *App) UpdatePaydexCoreInfo() {
	if a.config.PaydexCoreURL == "" {
		return
	}

	core := &paydexcore.Client{
		URL: a.config.PaydexCoreURL,
	}

	resp, err := core.Info(context.Background())
	if err != nil {
		log.Warnf("could not load paydex-core info: %s", err)
		return
	}
	...
	if resp.Info.Network != a.config.NetworkPassphrase {
		log.Errorf(
			"Network passphrase of paydex-core (%s) does not match Horizon configuration (%s). Exiting...",
			resp.Info.Network,
...
func (a *App) Tick() {
	...
	// update ledger state, operation fee state, and paydex-core info in parallel
	wg.Add(3)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdateFeeStatsState(); wg.Done() }()
	go func() { a.UpdatePaydexCoreInfo(); wg.Done() }()
	wg.Wait()
