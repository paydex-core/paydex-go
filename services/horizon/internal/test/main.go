...
import (
	...
	tdb "github.com/paydex-core/paydex-go/services/horizon/internal/test/db"
...
// LoadScenarioWithoutHorizon populates the test Paydex core database a with
...
func Start(t *testing.T) *T {
	...
	result.CoreDB = PaydexCoreDatabase(t)
...
// PaydexCoreDatabase returns a connection to the paydex core test database
//
// DEPRECATED:  use `PaydexCore()` from test/db package
func PaydexCoreDatabase(t *testing.T) *sqlx.DB {
	return tdb.PaydexCore(t)
}

// PaydexCoreDatabaseURL returns the database connection the url any test
// use when connecting to the paydex-core database
//
// DEPRECATED:  use `PaydexCoreURL()` from test/db package
func PaydexCoreDatabaseURL() string {
	return tdb.PaydexCoreURL()
}
