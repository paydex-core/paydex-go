...
// data produced by the connected paydex-core database, transforms it and
...
import (
	...
	ilog "github.com/paydex-core/paydex-go/support/log"
...
// Cursor iterates through a paydex core database's ledgers
type Cursor struct {
	...
	// CoreDB is the paydex-core db that data is ingested from.
	CoreDB *db.Session
	...
	// Name is a unique identifier tracking the latest ingested ledger on paydex-core
	Name string
...
type Config struct {
	...
	// CursorName is the cursor used for ingesting from paydex-core.
	// Setting multiple cursors in different Horizon instances allows multiple
	// Horizons to ingest from the same paydex-core instance without cursor
	// collisions.
	CursorName string
}
...
type System struct {
	...
	Network string
	// PaydexCoreURL is the http endpoint of the paydex-core that data is being
	// ingested from.
	PaydexCoreURL string
