...
import (
	...
	herr "github.com/paydex-core/paydex-go/services/horizon/internal/errors"
...
// RebaseHistory re-establishes horizon's history database by clearing it, ingesting the latest ledger in paydex-core then backfilling as many ledgers as possible
...
// run causes the importer to check paydex-core to see if we can import new
// data.
func (i *System) runOnce() {
	...
	if coreLatest == 1 {
		log.Warn("ingest: waiting for paydex-core sync")
		return
	}
