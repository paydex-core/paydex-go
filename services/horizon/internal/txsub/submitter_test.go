...
func TestDefaultSubmitter(t *testing.T) {
	ctx := test.Context()
	// submits to the configured paydex-core instance correctly
	...
	// Succeeds when paydex-core gives the DUPLICATE response.
	server = test.NewStaticMockServer(`{
				"status": "DUPLICATE",
				"error": null
				}`)
	...
	// Errors when the paydex-core url is empty

	s = NewDefaultSubmitter(http.DefaultClient, "")
	sr = s.Submit(ctx, "hello")
	assert.NotNil(t, sr.Err)

	//errors when the paydex-core url is not parseable

	s = NewDefaultSubmitter(http.DefaultClient, "http://Not a url")
	sr = s.Submit(ctx, "hello")
	assert.NotNil(t, sr.Err)

	// errors when the paydex-core url is not reachable
	s = NewDefaultSubmitter(http.DefaultClient, "http://127.0.0.1:65535")
	sr = s.Submit(ctx, "hello")
	assert.NotNil(t, sr.Err)

	// errors when the paydex-core returns an unparseable response
	server = test.NewStaticMockServer(`{`)
	defer server.Close()
	...
	// errors when the paydex-core returns an exception response
	...
	// errors when the paydex-core returns an unrecognized status
	...
	// errors when the paydex-core returns an error response
