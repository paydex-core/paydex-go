	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.assetType, func(t *testing.T) {
			...
			if testCase.valid {
				...
				tt.Equal("AssetType: paydex_asset_type does not validate as assetType", err.Error())
			}
		})
	}
}
...
func TestAccountIDValidator(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{
			"valid paydex address",
			"GAN4WOTCFSASG3J6SGLLQZURDDUVNBQANAHEQJ3PBNDZ74X63UZWQPZW",
			"",
		},
		{
			"empty paydex address should not be validated",
			"",
			"",
		},
	} {
