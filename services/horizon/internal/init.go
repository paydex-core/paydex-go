...
import (
	...
	ingestio "github.com/paydex-core/paydex-go/exp/ingest/io"
	...
	"github.com/paydex-core/paydex-go/services/horizon/internal/simplepath"
	"github.com/paydex-core/paydex-go/services/horizon/internal/txsub"
	results "github.com/paydex-core/paydex-go/services/horizon/internal/txsub/results/db"
...
func mustInitCoreDB(app *App) {
	session, err := db.Open("postgres", app.config.PaydexCoreDatabaseURL)
	if err != nil {
...
func initIngester(app *App) {
	...
	if app.config.NetworkPassphrase == "" {
		log.Fatal("Cannot start ingestion without network passphrase. Please confirm connectivity with paydex-core.")
	}

	app.ingester = ingest.New(
		app.config.NetworkPassphrase,
		app.config.PaydexCoreURL,
...
func initExpIngester(app *App, orderBookGraph *orderbook.OrderBookGraph) {
	...
	app.expingester, err = expingest.NewSystem(expingest.Config{
		...
		PaydexCoreURL:           app.config.PaydexCoreURL,
...
func initDbMetrics(app *App) {
	...
	app.metrics.Register("paydex_core.latest_ledger", app.coreLatestLedgerGauge)
	app.metrics.Register("history.open_connections", app.horizonConnGauge)
	app.metrics.Register("paydex_core.open_connections", app.coreConnGauge)
...
func initSubmissionSystem(app *App) {
	...
	app.submitter = &txsub.System{
		...
		Submitter:       txsub.NewDefaultSubmitter(http.DefaultClient, app.config.PaydexCoreURL),
