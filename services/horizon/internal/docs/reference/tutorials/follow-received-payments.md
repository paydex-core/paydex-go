...
using JavaScript and `EventSource`.  We will eschew using [`js-paydex-sdk`](https://github.com/paydex/js-paydex-sdk), the
...
```javascript
var Keypair = require("paydex-base").Keypair;
...
The Paydex test network provides the Friendbot, a tool that developers
can use to get testnet lumens for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://friendbot.paydex.org/?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Paydex network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://horizon-testnet.paydex.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/paydex/js-paydex-sdk).
...
var es = new EventSource('https://horizon-testnet.paydex.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
```bash
$ curl "https://horizon-testnet.paydex.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
```js
var PaydexBase = require("paydex-base");
PaydexBase.Network.useTestNetwork();

var keypair = PaydexBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new PaydexBase.Account(keypair.publicKey(), "713226564141056");

var amount = "100";
var transaction = new PaydexBase.TransactionBuilder(account)
  .addOperation(PaydexBase.Operation.createAccount({
    destination: PaydexBase.Keypair.random().publicKey(),
...
After running this script you should see a signed transaction blob. To submit this transaction we send it to horizon or paydex-core. But before we do, let's open a new console and start our previous script by `node stream_payments.js`.
