...
After the above are installed, run `go generate github.com/paydex-core/paydex-go/services/horizon/...`. This will look for any `.tmpl` files in the directory and use them to generate code when annotated structs are found in the package source.

## <a name="scenarios"></a> Adding, rebuilding and using test scenarios

In order to simulate ledgers Horizon uses [`paydex-core-commander`](https://github.com/paydex-core/paydex_core_commander) recipe files to add transactions and operations to ledgers using the paydex-core test framework.

In order to add a new scenario or rebuild existing scenarios you need:

1. [`paydex-core-commander`](https://github.com/paydex/paydex_core_commander) (in short: `scc`) installed and [configured](https://github.com/paydex/paydex_core_commander#assumptions-about-environment).
2. [`paydex-core`](https://github.com/paydex-core/paydex-core) binary.
3. This repository cloned locally.

`scc` allows you to write scripts/recipes that are later executed in `paydex-core` isolated network. After executing a recipe you can then export the `paydex-core` database to be able to run Horizon ingestion system against it (this repository contains a script that does this for you - read below).
...
account :scott, Paydex::KeyPair.from_seed("SBZWG33UOQQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAPSA")
account :bartek, Paydex::KeyPair.from_seed("SBRGC4TUMVVSAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCBDHV")
...
You can find more recipes in [`scc` examples](https://github.com/paydex-core/paydex_core_commander/tree/84d5ffb97202ecc3a0ed34a739c98e69536c0c2c/examples) and [horizon test scenarios](https://github.com/paydex-core/paydex-go/tree/master/services/horizon/internal/test/scenarios).

### Rebuilding scenarios

1. Create a new or modify existing recipe. All new recipes should be added to [horizon test scenarios](https://github.com/paydex-core/paydex-go/tree/master/services/horizon/internal/test/scenarios) directory.
2. In `paydex-core/paydex-go` repository root directory run `./services/horizon/internal/scripts/build_test_scenarios.bash`.
...
then, run the all the Go monorepo tests like so (assuming you are at paydex-core/paydex-go, or run from paydex-core/paydex-go/services/horizon for just the Horizon subset):
...
```bash
go test github.com/paydex-core/paydex-go/services/horizon/...
```

## <a name="logging"></a> Logging

All logging infrastructure is in the `github.com/paydex-core/paydex-go/tree/master/services/horizon/internal/log` package.  This package provides "level-based" logging:  Each logging statement has a severity, one of "Debug", "Info", "Warn", "Error" or "Panic".  The Horizon server has a configured level "filter", specified either using the `--log-level` command line flag or the `LOG_LEVEL` environment variable.  When a logging statement is executed, the statements declared severity is checked against the filter and will only be emitted if the severity of the statement is equal or higher severity than the filter.
