...
import (
	...
	horizon "github.com/paydex-core/paydex-go/services/horizon/internal"
	"github.com/paydex-core/paydex-go/services/horizon/internal/db2/schema"
	apkg "github.com/paydex-core/paydex-go/support/app"
	support "github.com/paydex-core/paydex-go/support/config"
...
var (
	...
	rootCmd = &cobra.Command{
		Use:   "horizon",
		Short: "client-facing api server for the paydex network",
		Long:  "client-facing api server for the paydex network. It acts as the interface between Paydex Core and applications that want to access the Paydex network. It allows you to submit transactions to the network, check the status of accounts, subscribe to event streams and more.",
...
func checkMigrations() {
	...
	if len(migrationsToApplyUp) > 0 {
		...
		stdLog.Printf("A database migration is required to run this version (%v) of Horizon. Run \"horizon db migrate up\" to update your DB. Consult the Changelog (https://github.com/paydex-core/paydex-go/blob/master/services/horizon/CHANGELOG.md) for more information.", apkg.Version())
		os.Exit(1)
	}

	nMigrationsDown := schema.GetNumMigrationsDown(config.DatabaseURL)
	if nMigrationsDown > 0 {
		stdLog.Printf("A database migration DOWN to an earlier version of the schema is required to run this version (%v) of Horizon. Consult the Changelog (https://github.com/paydex-core/paydex-go/blob/master/services/horizon/CHANGELOG.md) for more information.", apkg.Version())
...
var configOpts = []*support.ConfigOption{
	...
	&support.ConfigOption{
		Name:      "paydex-core-db-url",
		EnvVar:    "PAYDEX_CORE_DATABASE_URL",
		ConfigKey: &config.PaydexCoreDatabaseURL,
		OptType:   types.String,
		Required:  true,
		Usage:     "paydex-core postgres database to connect with",
	},
	&support.ConfigOption{
		Name:      "paydex-core-url",
		ConfigKey: &config.PaydexCoreURL,
		OptType:   types.String,
		Required:  true,
		Usage:     "paydex-core to connect with (for http commands)",
	},
	&support.ConfigOption{
		...
		Usage: "comma-separated list of paydex history archives to connect with",
	},
	&support.ConfigOption{
		Name:        "port",
	...
	&support.ConfigOption{
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes this horizon process to ingest data from paydex-core into horizon's db",
	},
	&support.ConfigOption{
	...
	&support.ConfigOption{
		...
		OptType:     types.String,
		FlagDefault: "HORIZON",
		Usage:       "ingestor cursor used by horizon to ingest from paydex core. must be uppercase and unique for each horizon instance ingesting from that core instance.",
	},
	&support.ConfigOption{
	...
	&support.ConfigOption{
		...
		OptType:     types.Uint,
		FlagDefault: uint(0),
		Usage:       "the maximum number of ledgers the history db is allowed to be out of date from the connected paydex-core db before horizon considers history stale",
	},
	&support.ConfigOption{
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes the ingester to skip reporting the last imported ledger state to paydex-core",
	},
	&support.ConfigOption{
