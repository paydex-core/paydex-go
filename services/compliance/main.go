...
import (
	...
	"github.com/paydex-core/paydex-go/services/compliance/internal/handlers"
	supportConfig "github.com/paydex-core/paydex-go/support/config"
	"github.com/paydex-core/paydex-go/support/db/schema"
	"github.com/paydex-core/paydex-go/support/errors"
	supportHttp "github.com/paydex-core/paydex-go/support/http"
...
func init() {
	rootCmd = &cobra.Command{
		Use:   "compliance",
		Short: "paydex compliance server",
		Long:  `paydex compliance server`,
		Run:   run,
	}
...
func NewApp(config config.Config, migrateFlag bool, versionFlag bool, version string) (app *App, err error) {
	...
	paydextomlClient := paydextoml.Client{
		HTTP: &httpClientWithTimeout,
	}

	federationClient := federation.Client{
		HTTP:        &httpClientWithTimeout,
		PaydexTOML: &paydextomlClient,
	}
	...
	err = g.Provide(
		...
		&inject.Object{Value: &paydextomlClient},
