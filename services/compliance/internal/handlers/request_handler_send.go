...
import (
	...
	"github.com/paydex-core/paydex-go/services/internal/bridge-compliance-shared/protocols/bridge"
...
func (rh *RequestHandler) HandlerSend(w http.ResponseWriter, r *http.Request) {
	...
	if authDataEntity != nil {
		var paydexToml *paydextoml.Response
		paydexToml, err = rh.PaydexTomlResolver.GetPaydexToml(authDataEntity.Domain)
		if err != nil {
		...
		if paydexToml.AuthServer == "" {
			log.Print("No AUTH_SERVER in paydex.toml")
			helpers.Write(w, callback.AuthServerNotDefined)
			return
		}

		rh.sendAuthData(w, paydexToml.AuthServer, []byte(authDataEntity.AuthData))
		return
	}
	...
	paydexToml, err := rh.PaydexTomlResolver.GetPaydexToml(domain)
	if err != nil {
		log.WithFields(log.Fields{
			"destination": request.Destination,
			"err":         err,
	...
	rh.sendAuthData(w, paydexToml.AuthServer, data)
}
