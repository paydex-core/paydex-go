...
func TestRequestHandlerAuthInvalidParams(t *testing.T) {
	...
	mockPaydextomlResolver.On(
		"GetPaydexTomlByAddress",
		"alice*paydex.org",
	).Return(&paydextoml.Response{
  ...
  "data": {
	...
	// When sender's paydex.toml does not contain signing key
	mockPaydextomlResolver.On(
		"GetPaydexTomlByAddress",
		"alice*paydex.org",
	).Return(&paydextoml.Response{}, nil).Once()
...
func TestRequestHandlerAuthSanctionsCheckNeedInfo(t *testing.T) {
	...
	mockDatabase.On(
		...
		"paydex.org", // sender = `alice*paydex.org`
	).Return(
