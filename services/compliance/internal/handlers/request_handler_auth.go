...
import (
	...
	baseAmount "github.com/paydex-core/paydex-go/amount"
	"github.com/paydex-core/paydex-go/protocols/compliance"
	...
	httpHelpers "github.com/paydex-core/paydex-go/services/internal/bridge-compliance-shared/http/helpers"
	callback "github.com/paydex-core/paydex-go/services/internal/bridge-compliance-shared/protocols/compliance"
...
func (rh *RequestHandler) HandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	senderPaydexToml, err := rh.PaydexTomlResolver.GetPaydexTomlByAddress(authData.Sender)
	if err != nil {
		log.WithFields(log.Fields{"err": err, "sender": authData.Sender}).Warn("Cannot get paydex.toml of sender")
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "Cannot get paydex.toml of sender")
		httpHelpers.Write(w, errorResponse)
		return
	}

	if !shared.IsValidAccountID(senderPaydexToml.SigningKey) {
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "SIGNING_KEY in paydex.toml of sender is invalid")
		// TODO
		// log.WithFields(errorResponse.LogData).Warn("SIGNING_KEY in paydex.toml of sender is invalid")
		httpHelpers.Write(w, errorResponse)
		return
	}
	...
	err = rh.SignatureSignerVerifier.Verify(senderPaydexToml.SigningKey, []byte(authreq.DataJSON), signatureBytes)
	if err != nil {
		log.WithFields(log.Fields{
			"signing_key": senderPaydexToml.SigningKey,
			"data":        authreq.Data,
