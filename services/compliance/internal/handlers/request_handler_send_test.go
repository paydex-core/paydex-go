...
import (
	...
	"github.com/paydex-core/paydex-go/protocols/federation"
	...
	"github.com/paydex-core/paydex-go/services/compliance/internal/mocks"
	"github.com/paydex-core/paydex-go/services/compliance/internal/test"
...
func TestRequestHandlerSendInvalidParams(t *testing.T) {
	...
	var mockPaydextomlResolver = new(mocks.MockPaydextomlResolver)
	...
	params := url.Values{
		...
		"sender":      {"alice*paydex.org"}, // GAW77Z6GPWXSODJOMF5L5BMX6VMYGEJRKUNBC2CZ725JTQZORK74HQQD
		"destination": {"bob*paydex.org"},   // GAMVF7G4GJC4A7JMFJWLUAEIBFQD5RT3DCB5DC5TJDEKQBBACQ4JZVEE
		"amount":      {"20"},
	}
	...
	params = url.Values{
		...
		// "source":      {"bad"},
		"sender":      {"alice*paydex.org"}, // GAW77Z6GPWXSODJOMF5L5BMX6VMYGEJRKUNBC2CZ725JTQZORK74HQQD
		"destination": {"bob*pyadex.org"},   // GAMVF7G4GJC4A7JMFJWLUAEIBFQD5RT3DCB5DC5TJDEKQBBACQ4JZVEE
		"amount":      {"20"},
	}
...
func TestRequestHandlerSendValidParams(t *testing.T) {
	...
	mockDatabase.On("InsertAuthData", mock.AnythingOfType("*db.AuthData")).Run(func(args mock.Arguments) {
		...
		assert.Equal(t, "paydex.org", entity.Domain)
	}).Return(nil).Once()
	...
	mockPaydextomlResolver.On(
		"GetPaydexToml",
		"paydex.org",
	).Return(&paydextoml.Response{AuthServer: authServer}, nil).Once()
	...
	mockHTTPClient.On(
		...
		url.Values{"address": {"alice*paydex.org"}},
	).Return(
	...
	params["forward_destination[domain]"] = []string{"paydex.org"}
