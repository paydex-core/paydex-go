...
func TestRequestHandlerPaymentErrorResponse(t *testing.T) {
	...
	mockFederationResolver.On(
		...
		errors.New("paydex.toml response status code indicates error"),
	).Once()
	...
	expected := test.StringToJSONMap(`{
		...
		"message": "Cannot resolve federated Paydex address."
	}`)
	...
	params = url.Values{
		"source":                      {"SDRAS7XIQNX25UDCCX725R4EYGBFYGJE4HJ2A3DFCWJIHMRSMS7CXX42"},
		"forward_destination[domain]": {"paydex.org"},
...
func TestRequestHandlerPaymentSuccessResponse(t *testing.T) {
	...
	// When destination is a paydex address it should return success
	params = url.Values{
		"source":      {"SDRAS7XIQNX25UDCCX725R4EYGBFYGJE4HJ2A3DFCWJIHMRSMS7CXX42"},
		"destination": {"bob*paydex.org"},
		"amount":      {"20.0"},
