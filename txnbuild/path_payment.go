...
// PathPayment represents the Paydex path_payment operation. This operation was removed
// in Paydex Protocol 12 and replaced by PathPaymentStrictReceive.
...
// PathPaymentStrictReceive represents the Paydex path_payment_strict_receive operation. See
...
func (pp *PathPaymentStrictReceive) Validate() error {
	err := validatePaydexPublicKey(pp.Destination)
	if err != nil {
		return NewValidationError("Destination", err.Error())
	}

	err = validatePaydexAsset(pp.SendAsset)
	if err != nil {
		return NewValidationError("SendAsset", err.Error())
	}

	err = validatePaydexAsset(pp.DestAsset)
	if err != nil {
