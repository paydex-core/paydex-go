...
func TestPathPaymentStrictSendValidateDestination(t *testing.T) {
	...
	err := tx.Build()
	if assert.Error(t, err) {
		expected := "validation failed for *txnbuild.PathPaymentStrictSend operation: Field: Destination, Error: SASND3NRUY5K43PN3H3HOP5JNTIDXJFLOKKNSCZQQAFBRSEIRD5OJKXZ is not a valid paydex public key"
		assert.Contains(t, err.Error(), expected)
	}
}
