...
func TestValidatePaydexPublicKey(t *testing.T) {
	validKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7YFMTLB65PYM"
	err := validatePaydexPublicKey(validKey)
	assert.NoError(t, err, "public key should be valid")

	invalidKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y"
	err = validatePaydexPublicKey(invalidKey)
	expectedErrMsg := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y is not a valid paydex public key"
	...
	err = validatePaydexPublicKey(invalidKey)
	expectedErrMsg = "SBCVMMCBEDB64TVJZFYJOJAERZC4YVVUOE6SYR2Y76CBTENGUSGWRRVO is not a valid paydex public key"
	require.EqualError(t, err, expectedErrMsg, "public key should be invalid")
}

func TestValidatePaydexAssetWithValidAsset(t *testing.T) {
	nativeAsset := NativeAsset{}
	err := validatePaydexAsset(nativeAsset)
	assert.NoError(t, err)

	kp0 := newKeypair0()
	creditAsset := CreditAsset{"XYZ", kp0.Address()}
	err = validatePaydexAsset(creditAsset)
	assert.NoError(t, err)
}

func TestValidatePaydexAssetWithInValidAsset(t *testing.T) {
	err := validatePaydexAsset(nil)
	assert.Error(t, err)
	...
	err = validatePaydexAsset(creditAssetNoCode)
	assert.Error(t, err)
	...
	err = validatePaydexAsset(creditAssetNoIssuer)
	assert.Error(t, err)
...
func TestValidateAmountInvalidValue(t *testing.T) {
	...
	require.EqualError(t, err, expectedErrMsg, "should be a valid paydex amount")
