/*
Package txnbuild implements transactions and operations on the Paydex network.
This library provides an interface to the Paydex transaction model. It supports the building of Go applications on
top of the Paydex network (https://www.paydex.org/). Transactions constructed by this library may be submitted
to any Horizon instance for processing onto the ledger, using any Paydex SDK client. The recommended client for Go
programmers is horizonclient (https://github.com/paydex-core/paydex-go/tree/master/clients/horizonclient). Together, these two
libraries provide a complete Paydex SDK.
For more information and further examples, see https://www.paydex.org/developers/go/reference/index.html.
*/
package txnbuild
...
// Account represents the aspects of a Paydex account necessary to construct transactions. See
// https://www.paydex.org/developers/guides/concepts/accounts.html
...
// Transaction represents a Paydex transaction. See
// https://www.paydex.org/developers/guides/concepts/transactions.html
...
// More details on SEP 10: https://github.com/paydex-core/paydex-protocol/blob/master/ecosystem/sep-0010.md
func BuildChallengeTx(serverSignerSecret, clientAccountID, anchorName, network string, timebound time.Duration) (string, error) {
	...
	sa := SimpleAccount{
		...
		//  (https://github.com/paydex-protocol/issues/1259)
		Sequence: int64(-1),
	}
	...
	// https://github.com/paydex-core/paydex-protocol/blob/master/ecosystem/sep-0010.md#response
	tx := Transaction{
		SourceAccount: &sa,
		Operations: []Operation{
			&ManageData{
...
// See description here: https://www.paydex.org/developers/guides/concepts/multi-sig.html#hashx.
...
// as a string. This can be used when you don't have access to a Paydex keypair.
...
func (tx *Transaction) SignWithKeyString(keys ...string) error {
	...
	for _, k := range keys {
		...
		if err != nil {
			return errors.Wrapf(err, "provided string %s is not a valid Paydex key", k)
		}
		kpf, ok := kp.(*keypair.Full)
		if !ok {
			return errors.New("provided string %s is not a valid Paydex secret key")
		}
...
// verifyTxSignature checks if a transaction has been signed by the provided Paydex account.
