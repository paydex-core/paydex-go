# Paydex Go 
[![Build Status](https://circleci.com/gh/paydex/go.svg?style=shield)](https://circleci.com/gh/paydex/go)
[![GoDoc](https://godoc.org/github.com/paydex/go?status.svg)](https://godoc.org/github.com/paydex/go)
[![Go Report Card](https://goreportcard.com/badge/github.com/paydex/go)](https://goreportcard.com/report/github.com/paydex/go)

This repo is the home for all of the public go code produced by SDF.  In addition to various tools and services, this repository is the SDK from which you may develop your own applications that integrate with the paydex network.

## Package Index

* [Horizon Server](services/horizon): Full-featured API server for Paydex network
* [Go Horizon SDK - horizonclient](clients/horizonclient): Client for Horizon server (queries and transaction submission)
* [Go Horizon SDK - txnbuild](txnbuild): Construct Paydex transactions and operations
* [Ticker](services/ticker): An API server that provides statistics about assets and markets on the Paydex network
* [Keystore](services/keystore): An API server that is used to store and manage encrypted keys for Paydex client applications
...
* **clients** contains packages that provide client packages to the various Paydex services.
* **exp** contains experimental packages.  Use at your own risk.
* **handlers** contains packages that provide pluggable implementors of `http.Handler` that make it easier to incorporate portions of the Paydex protocol into your own http server. 
* **support** contains packages that are not intended for consumption outside of Paydex's other packages.  Packages that provide common infrastructure for use in our services and tools should go here, such as `db` or `log`. 
...
In addition to the packages described above, this repository contains various packages related to working with the Paydex network from a go program.  It's recommended that you use [godoc](https://godoc.org/github.com/paydex/go#pkg-subdirectories) to browse the documentation for each.
