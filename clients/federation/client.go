...
// LookupByAddress performs a federated lookup following to the paydex
...
// "name" type is a legacy holdover from the legacy paydex network's federation
...
// LookupByAccountID performs a federated lookup following to the paydex
...
// ForwardRequest performs a federated lookup following to the paydex
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.PaydexTOML.GetPaydexToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get paydex.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("paydex.toml is missing federation server info")
	}
