...
// to sender server in order to obtain paydex.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderPaydexToml, err := paydextoml.GetPaydexTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get paydex.toml of sender domain")
	}

	if senderPaydexToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in paydex.toml of sender")
	}

	kp, err := keypair.Parse(senderPaydexToml.SigningKey)
	if err != nil {
