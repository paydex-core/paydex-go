...
type AuthData struct {
	// The paydex address of the customer that is initiating the send.
	Sender string `json:"sender" valid:"required,paydex_address"`
...
type AuthResponse struct {
	...
	// (only present if info_status is ok) JSON of the recipient's AML information. in the Paydex attachment convention
...
// Transaction represents transaction field in Paydex attachment
...
// Operation represents a single operation object in Paydex attachment
...
const (
	...
	// tx_status endpoint response. It represents that the
	// payment was approved by the receiving FI but the Paydex
	// transaction hasn't been received yet
	TransactionStatusApproved TransactionStatus = "approved"

	// TransactionStatusNotApproved is a value of `status` field for the
	// tx_status endpoint response. It represents that the
	// Paydex transaction was found but it was never approved
	// by the receiving FI.
