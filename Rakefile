...
namespace :xdr do
  ...
  # As paydex-core adds more .x files, we'll need to update this array
  # Prior to launch, we should be separating our .x files into a separate
  # repo, and should be able to improve this integration.
  HAYASHI_XDR = [
                 "src/xdr/Paydex-types.x",
                 "src/xdr/Paydex-ledger-entries.x",
                 "src/xdr/Paydex-transaction.x",
                 "src/xdr/Paydex-ledger.x",
                 "src/xdr/Paydex-overlay.x",
                 "src/xdr/Paydex-SCP.x",
                ]
  ...
  task :download do
    ...
    HAYASHI_XDR.each do |src|
      ...
      encoded    = client.contents("paydex/paydex-core", path: src).content
